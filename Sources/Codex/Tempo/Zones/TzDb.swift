...
extension Tempo {
  ...
  /// ``Tempo/ZoneRules`` provider for `tzdb`/`zoneinfo`.
  ///
  ...
  /// files are parsed, validated, and converted to ``Tempo/ZoneRules``
  ...
  public final class TzDb: ZoneRulesLoader {
    ...
    internal static let logger = Logger(subsystem: "io.codex.tempo", category: "TzDb")

    /// Errors related to loading ``Tempo/ZoneRules`` from `zoneinfo` data.
    ...
    ///   - retainParsedRules: If `true`, the parsed ``Tempo/TzIf/Rules``
    ///   data will be retained along with the ``Tempo/ZoneRules`` implementation.
    ...
    /// Loads a ``Tempo/ZoneRules`` implementation for the specified zone identifier.
    ///
    ...
    public func load(identifier: String) throws -> any ZoneRules {
      guard let entry = zones[identifier] else {
        throw Tempo.Error.invalidRegionalTimeZone(identifier: identifier)
      }
...
extension Tempo.ZoneRulesLoader where Self == Tempo.TzDb {

  public static var system: Self { Tempo.TzDb.default }
