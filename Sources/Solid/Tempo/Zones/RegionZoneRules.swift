...
extension Tempo {
  ...
  /// ``Tempo/FixedOffsetZoneRules`` class.
  ///
  public final class RegionZoneRules: ZoneRules, Sendable {
    ...
    ///
    /// - Note: The value is stored as both a ``Tempo/ZoneOffset`` and a
    /// ``Tempo/Duration`` for convenience in calculations.
    ///
    public let initial: (offset: ZoneOffset, duration: Duration, isStandard: Bool)
    /// The final offset for the region.
    ///
    /// - Note: The value is stored as both a ``Tempo/ZoneOffset`` and a
    /// ``Tempo/Duration`` to avoid recalculating the duration on each access.
    ///
...
extension Tempo.RegionZoneRules.TailRule {

  func offset(at instant: Tempo.Instant) -> Tempo.ZoneOffset {
  ...
  func nextTransition(at instant: Tempo.Instant) -> Tempo.Instant? {
  ...
  func prevTransition(at instant: Tempo.Instant) -> Tempo.Instant? {
    guard let (start, end, year) = computeStartEnd(at: instant) else {
      return nil
    }
  ...
  func computeStartEnd(
    at instant: Tempo.Instant
  ) -> (
    start: (instant: Tempo.Instant, rule: DateRule),
    end: (instant: Tempo.Instant, rule: DateRule),
    year: Int
  )? {
    ...
    let date = Tempo.GregorianCalendarSystem.default.localDate(instant: instant, offset: standardTime.offset)
...
extension Tempo.RegionZoneRules.TailRule.DateRule {

  // Compute the tail offset for a given year, rule, and offset.
  func tailOffset(for year: Int, offset: Tempo.ZoneOffset) -> Tempo.Instant {
    let cal = Tempo.GregorianCalendarSystem.default

    switch self {
    case .monthWeekDay(let m, let w, let d, let dayOffset):
      // 1) first day of month
      var date = Tempo.LocalDate(valid: (year: year, month: m, day: 1))
      ...
      date = Tempo.LocalDate(valid: (year: year, month: m, day: dayInMonth))
      // swift-format-ignore: NeverUseForceTry
      let offsetDate = try! date.adding(dayOffset).adding(-Tempo.Duration(offset))
    ...
    case .julianDay(let n, let leap, let dayOff):
      ...
      // swift-format-ignore: NeverUseForceTry
      let date = try! Tempo.LocalDate(year: year, ordinalDay: ordinal + 1)    // ordinal is 1-based
      // swift-format-ignore: NeverUseForceTry
      let offsetDate = try! date.adding(dayOff).adding(-Tempo.Duration(components: offset))
      return cal.instant(from: offsetDate, resolution: .default)
    }
  }
}
