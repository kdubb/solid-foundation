...
final class RFC4291IPv6AddressTests {
  ...
  func parseStandardIPv6Addresses() async throws {
    ...
    #expect(RFC4291.IPv6Address.parse(string: "2001:db8:85a3::8a2e:370:7334") != nil)
    #expect(RFC4291.IPv6Address.parse(string: "::1") != nil)
  ...
  func parseIPv6WithEmbeddedIPv4() async throws {
    #expect(RFC4291.IPv6Address.parse(string: "::ffff:192.168.1.1") != nil)
    #expect(RFC4291.IPv6Address.parse(string: "2001:db8::192.168.1.1") != nil)
  ...
  func verifyGroupValues() async throws {
    let address = RFC4291.IPv6Address.parse(string: "2001:0db8:85a3:0000:0000:8a2e:0370:7334")
    #expect(address != nil)
  ...
  func verifyCompressedAddressExpansion() async throws {
    let address = RFC4291.IPv6Address.parse(string: "2001:db8::8a2e:370:7334")
    #expect(address != nil)
    if let address = address {
      #expect(address.groups[0] == 0x2001)
      #expect(address.groups[1] == 0x0db8)
  ...
  func verifyEmbeddedIPv4Conversion() async throws {
    let address = RFC4291.IPv6Address.parse(string: "::ffff:192.168.1.1")
    #expect(address != nil)
    if let address = address {
  ...
  func verifySpecialAddressGroupValues() async throws {
    ...
    let loopback = RFC4291.IPv6Address.parse(string: "::1")
    #expect(loopback != nil)
    ...
    let unspecified = RFC4291.IPv6Address.parse(string: "::")
